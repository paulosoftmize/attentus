...
def main():
    ...
    hora_inicial = colunas[0].number_input(
        "Hora incial do dia (0-23)",
         min_value=0, max_value=23, value=0, step=1
    )

    minuto_incial = colunas[1].number_input(
    ...
    turnos_possiveis = lista_turnos_possiveis(1, qtd_horas_total,
                                              minutos_dividir)
    ...
    tempo_maximo_espera = st.sidebar.number_input(
        "Tempo máximo de espera para atendimentos (segundos)",
        min_value=0, max_value=1000, value=0, step=1
    )
    ...
    df_nad, df_tabela_ligantes = gera_tabelas(
        qtd_horas_total, minutos_dividir, hora_inicial, minuto_incial,
        turnos, encargos
    )
    ...
    arquivo_ligantes = st.sidebar.file_uploader(
        "Carregar arquivo de ligantes por período preenchidos",
        type=["xlsx"]
    )
    ...
    opcao = st.sidebar.segmented_control(
        options=["Ajustar Polinômio", "Rodar Attentus"],
    ...
    botao = st.sidebar.button("Avançar") # so vai rodar se clicar no botao, clicar na opção polinomio e fazer upload da tabela df_duracoes acho
     # aqui temmque escrver exatamente igual senao da erro ex. Polinõmio pos com acento la atras tem que por qui tb
    if opcao == "Ajustar Polinômio" and \
        botao and arquivo_duracoes:
        fig = ajusta_polinomio(df_duracoes, grau_polinomio)
        st.pyplot(fig)
    elif opcao == "Rodar Attentus" and \
        botao and arquivo_ligantes and arquivo_duracoes: # pq o Attentus depende dos dois arquivos -> arquivos_ligantes e arquivo_duracoes
        # print("RODAR MODELO!!!!!!!!!!!!!!!")
        ...
        coluna_nan = gerar_tabela_nan(df_tabela_ligantes,
                                      ...
                                      tempo_maximo_espera,
                                      tempo_descanso,
                                      grau_polinomio) # todos esses valores veem da tela
        ...
        incios = df_tabela_ligantes["hora_inicio"].to_dict()
        df_resultados = gera_resultados(prob, regimes, incios)
